  ...
  if(isset($_POST["add_to_cart"])){
    ...
    $sql = "INSERT INTO cart(album_name,price,quantity,customer_id) VALUES('$album_name','$price','$quantity','$customer_id')";
      mysqli_query($db, $sql);
       ...
       echo "<script>window.location('pop.php')</script>";
      } 
    else
    {
...
<?php
 ...
 $email=$_SESSION['email'];
  $sql = "SELECT A.*,L.customer_id FROM ALBUM A,LoginCredentials L where A.genre_id=2 and L.email='$email' order by A.album_id ASC";
  
    $result= mysqli_query($db,$sql);}
    else{
  $sql1="SELECT A.* FROM ALBUM A WHERE A.genre_id=2 order by A.album_id ASC";
